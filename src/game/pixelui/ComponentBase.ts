...
export class ComponentBase extends Phaser.GameObjects.Container {
  ...
  private style: ComponentBaseStyle | undefined;
  ...
  private tween: Phaser.Tweens.Tween | undefined = undefined;
  ...
  constructor(
    ...
    /* Colors */
    const fillColor = Phaser.Display.Color.ValueToColor(style?.fillColor || PixelUI.theme.backgroundColor());
    const fillLockColor = Phaser.Display.Color.ValueToColor(
      style?.fillLockColor || PixelUI.theme.styles.colorMain || 0,
    );
    const borderMainColor = Phaser.Display.Color.ValueToColor(style?.borderColor || PixelUI.theme.borderColor());
    ...
    /* Size */
    const width = style?.fixedWidth || 0;
    const height = style?.fixedHeight || 0;

    /* Shadow */
    const shadow = scene.add.rectangle(4, 4, width + 8, height + 8, shadowColor.color, shadowAlpha);
    shadow.setOrigin(align);

    /* Base rectangle */
    const fill = scene.add.rectangle(0, 0, width, height, fillColor.color, fillColor.alphaGL);
    ...
    borderEdge.setStrokeStyle(4, borderEdgeColor.color, borderEdgeColor.alphaGL || 1.0);
    borderEdge.setOrigin(align);

    const borderMain = scene.add.rectangle(0, 0, width, height);
    borderMain.setStrokeStyle(4, borderMainColor.color, borderMainColor.alphaGL || 1.0);
    borderMain.setFillStyle();
    borderMain.setOrigin(align);

    const base = scene.add.container(0, 0, [fill, scene.add.container(0, 0, children), borderEdge, borderMain]);
  ...
  public highlight(hover: boolean): void {
    this.borderMain.strokeColor = hover ? this.borderHoverColor.color : this.borderMainColor.color;
  }
}
