...
export class Theme {
  ...
  public backgroundColor(localTheme?: ThemeStyles): string {
    const color = this.compose(localTheme).themeDark
      ? Phaser.Display.Color.ValueToColor(this.compose(localTheme).colorDarkShade || 0).lighten(10)
      : Phaser.Display.Color.ValueToColor(this.compose(localTheme).colorLightShade || 0).darken(10);
    return color.rgba;
  }
  ...
  public borderColor(localTheme?: ThemeStyles): string {
    const color = Phaser.Display.Color.ValueToColor(this.compose(localTheme).colorLightShade || 0);
    return color.rgba;
  }
  ...
  public borderEdgeColor(localTheme?: ThemeStyles): string {
    const color = Phaser.Display.Color.ValueToColor(this.compose(localTheme).colorLightShade || 0).darken(60);
    return color.rgba;
  }
  ...
  public borderHoverColor(localTheme?: ThemeStyles): string {
    const color = this.compose(localTheme).themeDark
      ? Phaser.Display.Color.ValueToColor(this.compose(localTheme).colorDarkAccent || 0).lighten(50)
      : Phaser.Display.Color.ValueToColor(this.compose(localTheme).colorLightAccent || 0);
    return color.rgba;
  }
