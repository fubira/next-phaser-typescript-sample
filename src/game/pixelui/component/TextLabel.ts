...
export interface TextLabelStyle extends Phaser.Types.GameObjects.Text.TextStyle {
...
export class TextLabel extends Phaser.GameObjects.Text {
  constructor(scene: Phaser.Scene, x: number, y: number, text: string | string[], style: PixelUI.TextLabelStyle = {}) {
    const theme = PixelUI.theme.styles;
    const textSize = PixelUI.theme.getTextSize(style.textSize || 'normal');

    if (!style.color) {
      style.color = theme.textStroke ? theme.colorLightShade || '#FFFFFF' : theme.colorDarkShade || '#000000';
    }
