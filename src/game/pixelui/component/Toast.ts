...
class ToastQueue {
  ...
  private relocateAll(): void {
    ...
    const topToast = this.queue.filter((toast: Toast) => toast.verticalAlign === 'top');
    yBase = 0;
    ...
    const middleToast = this.queue.filter((toast: Toast) => toast.verticalAlign === 'middle');
    yBase = 0;
    ...
    const bottomToast = this.queue.filter((toast: Toast) => toast.verticalAlign === 'bottom');
    yBase = 0;
...
export class Toast extends ComponentBase {
  ...
  constructor(scene: Phaser.Scene, title?: string, message?: string | string[], style?: PixelUI.ToastStyle) {
    const maxWidth = scene.scale.width * 0.95;
    const maxHeight = scene.scale.height * 0.8;
    ...
    const borderColor = Phaser.Display.Color.ValueToColor(PixelUI.theme.styles.colorLightShade || 0).lighten(50);

    let typeColor = Phaser.Display.Color.ValueToColor(PixelUI.theme.styles.colorMain || 0);

    switch (style?.type) {
      case 'success': {
        typeColor = Phaser.Display.Color.ValueToColor(PixelUI.theme.styles.colorSuccess || 0);
        break;
      }
      case 'error': {
        typeColor = Phaser.Display.Color.ValueToColor(PixelUI.theme.styles.colorDanger || 0);
        break;
      }
      case 'warning': {
        typeColor = Phaser.Display.Color.ValueToColor(PixelUI.theme.styles.colorWarning || 0);
        break;
      }
    }
    ...
    const strokeThickness = PixelUI.theme.getTextSize(textSize) / 8;
    ...
    const typeBar = scene.add.rectangle(0, 0, 16, 16, typeColor.color, typeColor.alphaGL);
    ...
    typeBar.setPosition(-fixedWidth / 2 + 6, 0);
