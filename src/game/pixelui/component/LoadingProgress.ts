...
export class LoadingProgress extends Phaser.GameObjects.Container {
  ...
  constructor(
    ...
    if (spriteFrame instanceof Phaser.GameObjects.Sprite) {
      ...
      this.spriteFrame = new Phaser.GameObjects.Sprite(scene, 0, 0, spriteFrame);
    }
   ...
   */
  public start(load: () => void): Promise<void> {
 ...
 */
export function LoadingProgressFactory(scene: Phaser.Scene, textureBar: string, textureFrame: string): LoadingProgress {
